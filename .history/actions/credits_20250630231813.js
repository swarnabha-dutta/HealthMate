...
export async function deductCreditsForPayout(doctorId, creditsToDeduct) {
    try {
        const doctor = await db.user.findUnique({ where: { id: doctorId } });

        if (!doctor) throw new Error("Doctor not found");
        if (doctor.credits < creditsToDeduct) throw new Error("Insufficient credits for payout");
        ...
        console.error("Failed to deduct credits for payout:", error);
        return { success: false, error: error.message };
    }
}